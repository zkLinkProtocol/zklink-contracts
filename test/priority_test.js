...
describe('ZkLink priority queue ops unit tests', function () {
    ...
    it('invalid state or params should be failed when deposit', async () => {
        ...
        await token2.connect(defaultSender).approve(zkLink.target, 100);
        await expect(zkLink.connect(defaultSender).depositERC20(token2.target, 30, extendAddress(to), 0, false)).to.be.revertedWith("0");
        ...
        await tokenNotRegistered.connect(defaultSender).approve(zkLink.target, 100);
        await expect(zkLink.connect(defaultSender).depositERC20(tokenNotRegistered.target, 30, extendAddress(to), 0, false)).to.be.revertedWith("e3");

        // token deposit paused
        await periphery.connect(governor).setTokenPaused(token2Id, true);
        await expect(zkLink.connect(defaultSender).depositERC20(token2.target, 30, extendAddress(to), 0, false)).to.be.revertedWith("e4");
        await periphery.connect(governor).setTokenPaused(token2Id, false);

        // token mapping not supported
        await expect(zkLink.connect(defaultSender).depositERC20(token2.target, 30, extendAddress(to), 0, true)).to.be.revertedWith("e5");

        // zero amount
        await expect(zkLink.connect(defaultSender).depositETH(extendAddress(to), subAccountId, {value: 0})).to.be.revertedWith("e0");

        // zero to address
        await expect(zkLink.connect(defaultSender).depositETH(extendAddress(hardhat.ethers.ZeroAddress), subAccountId, {value: amount})).to.be.revertedWith("e1");
    ...
    it('deposit eth should success', async () => {
        const balance0 = await ethers.provider.getBalance(zkLink.target);
        const to = "0x72847C8Bdc54b338E787352bceC33ba90cD7aFe0";
        const subAccountId = 0;
        const amount = parseEther("1");
        await zkLink.connect(defaultSender).depositETH(extendAddress(to), subAccountId, {value: amount});
        const balance1 = await ethers.provider.getBalance(zkLink.target);
        expect(balance1 - balance0).eq(amount);
    ...
    it('deposit standard erc20 should success', async () => {
        ...
        let contractBalance = await token2.balanceOf(zkLink.target);
        await token2.connect(defaultSender).approve(zkLink.target, 100);
        await zkLink.connect(defaultSender).depositERC20(token2.target, amount, extendAddress(to), subAccountId, false);
        expect(await token2.balanceOf(zkLink.target)).equal(contractBalance + BigInt(amount));
        expect(await token2.balanceOf(defaultSender.address)).equal(senderBalance - BigInt(amount));
    ...
    it('deposit erc20 with mapping should success', async () => {
        ...
        let contractBalance = await token4.balanceOf(zkLink.target);
        await token4.connect(defaultSender).approve(zkLink.target, 100);
        await zkLink.connect(defaultSender).depositERC20(token4.target, amount, extendAddress(to), subAccountId, true);
        expect(await token4.balanceOf(zkLink.target)).equal(contractBalance + BigInt(amount));
        expect(await token4.balanceOf(defaultSender.address)).equal(senderBalance - BigInt(amount));
    ...
    it('deposit standard erc20 with decimals should success', async () => {
        ...
        let contractBalance = await token5.balanceOf(zkLink.target);
        await token5.connect(defaultSender).approve(zkLink.target, parseUnits("100000000", "wei")); // 100 * 10 ^6
        await zkLink.connect(defaultSender).depositERC20(token5.target, amount, extendAddress(to), subAccountId, false);
        expect(await token5.balanceOf(zkLink.target)).equal(contractBalance + BigInt(amount));
        expect(await token5.balanceOf(defaultSender.address)).equal(senderBalance - BigInt(amount));
