...
describe('Accept unit tests', function () {
    ...
    it('invalid state or params should failed when accept', async () => {
        ...
        await expect(periphery.connect(alice).acceptERC20(alice.address, fwAId, bob.address, 10000, 100, 20, 1, 10, 0))
    ...
    it('accept eth should success', async () => {
        ...
        await expect(periphery.connect(alice).acceptETH(alice.address, fwAId, bob.address, amount, feeRate, accountIdOfNonce, subAccountIdOfNonce, nonce, {value: amountReceive}))
            ...
            .withArgs(alice.address, fwAId, bob.address, ethId, amount, feeRate, accountIdOfNonce, subAccountIdOfNonce, nonce, amountReceive);
    ...
    it('accept standard erc20 should success', async () => {
        ...
        await expect(periphery.connect(bob).acceptERC20(bob.address, fwAId, alice.address, token2Id, amount, feeRate, accountIdOfNonce, subAccountIdOfNonce, nonce))
            .to.be.emit(periphery, "Accept")
            .withArgs(bob.address, fwAId, alice.address, token2Id, amount, feeRate, accountIdOfNonce, subAccountIdOfNonce, nonce, amountReceive);
        ...
        await expect(periphery.connect(defaultSender).acceptERC20(bob.address, fwAId, alice.address, token2Id, amount, feeRate, accountIdOfNonce, subAccountIdOfNonce, nonce))
            .to.be.emit(periphery, "Accept")
            .withArgs(bob.address, fwAId, alice.address, token2Id, amount, feeRate, accountIdOfNonce, subAccountIdOfNonce, nonce, amountReceive);
