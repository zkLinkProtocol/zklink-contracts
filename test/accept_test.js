...
describe('Accept unit tests', function () {
    ...
    it('accept eth should success', async () => {
        ...
        const feeRate = 100; // 1%
        let nonceFromAccountId = 10;
        let nonceFromSubAccountId = 0;
        let nonce = 1;
        const amountReceive = parseEther("0.99");
        await expect(periphery.connect(alice).acceptETH(alice.address, fwAId, bob.address, amount, feeRate, nonceFromAccountId, nonceFromSubAccountId, nonce, {value: amountReceive}))
            .to.be.emit(periphery, "Accept")
            .withArgs(alice.address, fwAId, bob.address, ethId, amountReceive, amountReceive);
        let hash = calAcceptHash(bob.address, ethId, amount, feeRate, nonceFromAccountId, nonceFromSubAccountId, nonce);
        ...
        await expect(periphery.connect(alice).acceptETH(alice.address, fwAId, bob.address, amount, feeRate, nonceFromAccountId, nonceFromSubAccountId, nonce, {value: amountSentNotEnough}))
            .to.be.reverted;

        // send more
        const amountSentMore = parseEther("1.03");
        await expect(periphery.connect(alice).acceptETH(alice.address, fwAId, bob.address, amount, feeRate, nonceFromAccountId, nonceFromSubAccountId, nonce, {value: amountSentMore}))
            .to.be.emit(periphery, "Accept")
            .withArgs(alice.address, fwAId, bob.address, ethId, amountReceive, amountReceive);
        ...
        nonce = 3;
        await expect(periphery.connect(alice).acceptETH(alice.address, fwAId, periphery.address, amount, feeRate, nonceFromAccountId, nonceFromSubAccountId, nonce, {value: amountReceive}))
            .to.be.reverted;

        // msg sender is not the acceptor
        nonce = 4;
        await expect(periphery.connect(defaultSender).acceptETH(alice.address, fwAId, bob.address, amount, feeRate, nonceFromAccountId, nonceFromSubAccountId, nonce, {value: amountReceive}))
            .to.be.emit(periphery, "Accept")
            .withArgs(alice.address, fwAId, bob.address, ethId, amountReceive, amountReceive);
    });
    ...
    it('accept standard erc20 should success', async () => {
        const amount = parseEther("1");
        const feeRate = 100; // 1%
        let nonceFromAccountId = 15;
        let nonceFromSubAccountId = 3;
        let nonce = 1;
        const amountReceive = parseEther("0.99");
        await token2.connect(bob).mint(parseEther("100"));
        await token2.connect(bob).approve(periphery.address, amount);
        await expect(periphery.connect(bob).acceptERC20(bob.address, fwAId, alice.address, token2Id, amount, feeRate, nonceFromAccountId, nonceFromSubAccountId, nonce, amountReceive))
            .to.be.emit(periphery, "Accept")
            .withArgs(bob.address, fwAId, alice.address, token2Id, amountReceive, amountReceive);
        let hash = calAcceptHash(alice.address, token2Id, amount, feeRate, nonceFromAccountId, nonceFromSubAccountId, nonce);
        ...
        nonce = 2;
        await expect(periphery.connect(bob).acceptERC20(bob.address, fwAId, alice.address, token2Id, amount, feeRate, nonceFromAccountId, nonceFromSubAccountId, nonce, parseEther("0.98")))
            .to.be.reverted;
        ...
        await expect(periphery.connect(defaultSender).acceptERC20(bob.address, fwAId, alice.address, token2Id, amount, feeRate, nonceFromAccountId, nonceFromSubAccountId, nonce, amountReceive))
            .to.be.emit(periphery, "Accept")
            .withArgs(bob.address, fwAId, alice.address, token2Id, amountReceive, amountReceive);
    ...
    it('accept non standard erc20 should success', async () => {
        ...
        await expect(periphery.connect(bob).acceptERC20(bob.address, fwAId, alice.address, token3Id, amount, feeRate, nonceFromAccountId, nonceFromSubAccountId, nonce, amountTransfer))
            .to.be.emit(periphery, "Accept")
