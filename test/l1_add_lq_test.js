...
describe('L1AddLQ unit tests', function () {
    ...
    it('cancelOutstandingDepositsForExodusMode should success', async () => {
        ...
        const pubdata = getL1AddLQPubdata({ owner:bob.address, chainId:'0x00', tokenId:'0x0001', amount:'0x00000000000000000000000000000014', pair:pair.address, lpAmount:'0x00000000000000000000000000000000', nftTokenId:'0x00000001' });
        await zkSync.setExodusMode(true);
    ...
    it('only current chain will handle add lq op', async () => {
        ...
        const pubdata = getL1AddLQPubdata({ owner:bob.address, chainId:'0x01', tokenId:'0x0001', amount:'0x00000000000000000000000000000014', pair:pair.address, lpAmount:'0x00000000000000000000000000000000', nftTokenId:'0x00000001' });
        await zkSyncBlock.testExecL1AddLQ(pubdata);
        const nftInfo = await nft.tokenLq(1);
