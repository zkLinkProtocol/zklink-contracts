...
contract Verifier is KeysWithPlonkVerifier, KeysWithPlonkVerifierOld {
    ...
    function verifyExitProof(bytes32 _rootHash, uint8 _chainId, uint32 _accountId, uint8 _subAccountId, address _owner, uint16 _tokenId, uint16 _srcTokenId, uint128 _amount, uint256[] calldata _proof) external virtual view returns (bool) {
        bytes12 addressPrefixZeroBytes = bytes12(0);
        bytes32 commitment = sha256(abi.encodePacked(_rootHash, _chainId, _accountId, _subAccountId, addressPrefixZeroBytes, _owner, _tokenId, _srcTokenId, _amount));
